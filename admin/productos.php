...
$CATEGORIAS = [
  ...
  'quirurgico'    => 'Línea Quirúrgica',
  'kit_pacientes' => 'Kits Pacientes',
  'cobertores'    => 'Cobertores',
  'ambos'         => 'Ambos',
  'kit_dialisis'  => 'Kits Diálisis',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($postAction === 'delete') {
    $id  = (int)($_POST['id'] ?? 0);
    $row = $db->prepare('SELECT imagen, imagenes FROM productos WHERE id = ?');
    $row->execute([$id]);
    $prod = $row->fetch();
    if ($prod) {
      if ($prod['imagen']) {
      ...
      foreach ($imagenes as $img) {
        $imgPath = __DIR__ . '/../' . $img;
        if (file_exists($imgPath) && strpos($img, 'dynamic/') !== false) {
          unlink($imgPath);
        }
      }
    }
    ...
    if ($postAction === 'create') {
      ...
      if (!empty($_FILES['galeria']['name'][0])) {
        ...
        for ($i = 0; $i < $total; $i++) {
          if ($_FILES['galeria']['error'][$i] === UPLOAD_ERR_OK && $_FILES['galeria']['size'][$i] > 0) {
            $fileData = [
            ...
            $uploaded = handleImageUpload($fileData, $uploadDir);
            if ($uploaded) {
              $imagenes[] = $uploaded;
            }
          }
      ...
      if ($nombre && $categoria) {
        $stmt = $db->prepare('INSERT INTO productos (nombre, nombre_en, nombre_pt, categoria, desc_corta, desc_corta_en, desc_corta_pt, descripcion, descripcion_en, descripcion_pt, imagen, imagenes, specs, specs_en, specs_pt, activo, orden) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
        $stmt->execute([$nombre, $nombre_en, $nombre_pt, $categoria, $descCorta, $descCorta_en, $descCorta_pt, $desc, $desc_en, $desc_pt, $imagen, $imagenesJson, $specsJson, $specsJson_en, $specsJson_pt, $activo, $orden]);
      ...
      $cur = $db->prepare('SELECT imagen, imagenes FROM productos WHERE id = ?');
      $cur->execute([$id]);
      $curData = $cur->fetch();
      $imagen  = $curData['imagen'] ?? '';
      $imagenes = json_decode($curData['imagenes'] ?? '[]', true);

      $deleteGaleria = $_POST['delete_galeria'] ?? [];
      if (!empty($deleteGaleria)) {
        $newImagenes = [];
        foreach ($imagenes as $img) {
          if (in_array($img, $deleteGaleria)) {
            $imgPath = __DIR__ . '/../' . $img;
            if (file_exists($imgPath) && strpos($img, 'dynamic/') !== false) {
              unlink($imgPath);
            }
      ...
      if ($nombre && $categoria) {
        $stmt = $db->prepare('UPDATE productos SET nombre=?, nombre_en=?, nombre_pt=?, categoria=?, desc_corta=?, desc_corta_en=?, desc_corta_pt=?, descripcion=?, descripcion_en=?, descripcion_pt=?, imagen=?, imagenes=?, specs=?, specs_en=?, specs_pt=?, activo=?, orden=?, updated_at=CURRENT_TIMESTAMP WHERE id=?');
        $stmt->execute([$nombre, $nombre_en, $nombre_pt, $categoria, $descCorta, $descCorta_en, $descCorta_pt, $desc, $desc_en, $desc_pt, $imagen, $imagenesJson, $specsJson, $specsJson_en, $specsJson_pt, $activo, $orden, $id]);
  ...
  <div class="flex min-h-screen">
    <aside class="w-64 bg-dark-navy text-white flex flex-col shrink-0">
      ...
      <nav class="flex-1 px-4 py-6 space-y-1">
        ...
        <a href="mensajes.php" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-slate-400 hover:bg-slate-800 hover:text-white transition-colors">
          <span class="material-symbols-outlined">mail</span> Mensajes
        </a>
        
        <div class="pt-6 mt-6 border-t border-white/5">
          <a href="../index.php" target="_blank" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-slate-400 hover:text-white transition-colors">
            <span class="material-symbols-outlined">public</span> Ver Sitio Web
          </a>
        </div>
      </nav>
      <div class="px-4 pb-6">
      ...
      <?php elseif ($action === 'new' || $action === 'edit'): ?>
        ...
        <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <h3 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-5">Imagen Principal</h3>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <h3 class="text-xs font-black uppercase tracking-widest text-slate-400 mb-5">Galería Adicional</h3>
              <?php
              if ($isEdit):
                  ...
                  <div class="grid grid-cols-3 gap-2 mb-4">
                    <?php foreach ($imagenes as $img): ?>
                      <div class="relative group">
                        <img src="../<?= htmlspecialchars($img) ?>" alt="" class="w-full h-16 object-cover rounded bg-slate-50 border border-slate-200">
                        <label class="absolute top-1 right-1 bg-white/90 rounded p-0.5 cursor-pointer flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity border border-red-200" title="Eliminar imagen">
                          <input type="checkbox" name="delete_galeria[]" value="<?= htmlspecialchars($img) ?>" class="w-3 h-3 text-red-500 rounded border-slate-300">
                        </label>
                      </div>
                    <?php endforeach; ?>
                  </div>
                  <p class="text-[10px] text-slate-400 mb-4">Seleccioná la casilla de una imagen para eliminarla al guardar.</p>
              <?php
                endif;
              endif;
              ?>
              <input type="file" name="galeria[]" multiple accept="image/*" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-600 hover:file:bg-slate-200">
            </div>
